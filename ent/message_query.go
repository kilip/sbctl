...
func (_q *MessageQuery) QueryMemories() *MemoryQuery {
...
func (_q *MessageQuery) WithMemories(opts ...func(*MemoryQuery)) *MessageQuery {
	query := (&MemoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if query := _q.withMemories; query != nil {
		if err := _q.loadMemories(ctx, query, nodes,
			func(n *Message) { n.Edges.Memories = []*Memory{} },
			func(n *Message, e *Memory) { n.Edges.Memories = append(n.Edges.Memories, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *MessageQuery) loadMemories(ctx context.Context, query *MemoryQuery, nodes []*Message, init func(*Message), assign func(*Message, *Memory)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(memory.FieldMessageID)
	}
	query.Where(predicate.Memory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(message.MemoriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
